...
| Fonts and other UI assets must be owned independently of ROM media | UTF-8 Japanese names are correct in `library-index.json`, but font lookup followed SD2 and fell back to built-in ASCII. | **device backend verified, physical text pending**: FE loaded `/storage/plumos/fonts/default.otf` and `cjk-fallback.ttc`; build probes Japanese glyphs and the 599-ROM index retains Japanese names. |

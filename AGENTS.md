...
- FEとエミュレーター、アプリ、診断用レンダラーなどが同時に画面へ描画すると、
  画面の点滅やLCDのimage retentionを発生させる可能性がある。実機上でこれらを
  起動する場合は、必ずFEが提供する正規の起動経路を使用するか、FEプロセスが完全に
  終了し、`/dev/fb0`や`/dev/dri/card*`などの画面デバイスを解放していることを確認してから
  実行する。`SIGSTOP`などでFEを一時停止しただけの状態を「停止」とみなしてはならない。

...
grep -q 'PLUMOS_PROVISION_FAST_BOOT="$clean_fast_boot"' "$repo_root/rootfs/bubble-external-initramfs/init"
...
grep -q '^normal_boot=paired-clean-shutdown-markers-skip-filesystem-repair$' \

...
SOURCE="$ROOT_DIR/package/portmaster-bubble/src/plumos_portmaster_exec_guard.c"
...
run_test() {
    ...
    grep -q "^LD_PRELOAD=/port/scaler.so:$guard$" "$output"
    grep -q '^PLUMOS_PORTMASTER_SESSION_ID=bubble-test-session$' "$output"
}
...
if [[ "$(uname -s)" == Darwin ]]; then
    ...
    docker run --rm \
        -v "$ROOT_DIR:/repo:ro" \
        plumos-bubble-tools:dev \
        /repo/tests/test-portmaster-bubble-exec-guard.sh
else
...
printf 'portmaster_bubble_exec_guard=result-ok\n'

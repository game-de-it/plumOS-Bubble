#!/usr/bin/env python3
"""Transactional Runtime updater for plumOS Bubble."""
...
PLUMOS_ROOT = Path(os.environ.get("PLUMOS_ROOT", "/storage/plumos"))
USER_ROOT = Path(os.environ.get("PLUMOS_USERDATA_ROOT", "/storage/user"))
...
PUBLIC_KEY = Path(os.environ.get("PLUMOS_UPDATE_PUBLIC_KEY", str(PLUMOS_ROOT / "share/update/plumos-bubble-update-public.pem")))
OPENSSL = os.environ.get("PLUMOS_UPDATE_OPENSSL", str(PLUMOS_ROOT / "bin/plumos-openssl"))
...
MANAGED_CONFIG_PREFIXES = (
    ...
    "config/frontend/boot-splash.png",
    "config/frontend/menus.json",
    "config/frontend/scraper-sources.tsv",
    "config/frontend/runtime-coverage.json",
    "config/frontend/start-menu-coverage.json",
...
def validate_manifest(manifest: dict[str, Any], archive_names: set[str]) -> None:
    ...
    if manifest.get("package_type") != "runtime":
        raise UpdateError("Bubble currently accepts runtime update packages only")
    if manifest.get("device_id") != DEVICE_ID or manifest.get("architecture") != ARCHITECTURE:
        raise UpdateError("package target does not match Bubble aarch64")
    ...
    for relative in manifest.get("delete", []):
        ...
        if relative in declared:
            raise UpdateError(f"payload path is also marked for deletion: {relative}")
        if not managed_runtime_path(relative):
...
def current_compatibility(manifest: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise UpdateError("installed ABI metadata is invalid") from exc
    try:
        compatible = int(manifest["system_abi_min"]) <= system_abi <= int(manifest["system_abi_max"])
        message = "system ABI is outside the runtime package range"
    except (KeyError, TypeError, ValueError) as exc:
        raise UpdateError("package ABI metadata is invalid") from exc
    if not compatible:
...
def scan_packages() -> list[dict[str, Any]]:
    ...
    if not inbox.is_dir():
        return results
    for path in sorted(inbox.glob("plumos-bubble-*.tar.gz")):
        try:
            item = inspect(path)
...
def rollback_runtime(reason: str) -> None:
    ...
    for operation in reversed(journal.get("operations", [])):
        ...
        if operation.get("existed") and backup_exists:
            if target_exists:
                remove_path(target)
            target.parent.mkdir(parents=True, exist_ok=True)
            os.replace(backup, target)
        elif not operation.get("existed") and target_exists:
            remove_path(target)
...
def apply_runtime(package: Path, manifest: dict[str, Any]) -> int:
    ...
    # Persist the complete rollback plan once before changing any live path.
    # Rollback derives per-path progress from the atomic target/backup layout,
    # so it does not need to rewrite an ever-growing JSON journal for every
    # small file.
    journal_write(journal)
    try:
...
def main() -> int:
    ...
    read_only_command = args.command in {"inspect", "scan"}
    try:

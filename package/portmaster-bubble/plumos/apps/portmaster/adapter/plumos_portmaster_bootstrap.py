#!/usr/bin/env python3
"""Start the official PortMaster GUI with the plumOS Bubble hardware contract."""
...
APP_ROOT = Path(
    os.environ.get(
        "PLUMOS_PORTMASTER_APP_ROOT", "/storage/plumos/apps/portmaster"
    )
)
DATA_ROOT = Path(
    os.environ.get(
        "PLUMOS_PORTMASTER_DATA_ROOT",
        "/storage/plumos/state/portmaster/data",
    )
)
...
def install_bubble_contract() -> None:
    ...
    hardware.DEVICES["GKD Bubble"] = {
        "device": "miyoo-flip",
        "manufacturer": "Miyoo",
        "cfw": ["plumOS"],
    }
    hardware.HW_INFO["miyoo-flip"] = {
        "resolution": (640, 480),
        "analogsticks": 2,
        "cpu": "rk3566",
        "capabilities": [],
    ...
    harbourmaster.HW_INFO["miyoo-flip"] = hardware.HW_INFO["miyoo-flip"]

...
/*
 * DraStic 2.5.0.4 creates small POSIX shared-memory files and subsequently
 * maps the complete emulated RAM/VRAM ranges from them.  The Miyoo release
 * kernel permits this, but the Bubble plumOS userspace receives SIGBUS as soon as
 * the core touches a page beyond the file's current end.  Extend only the two
 * exact DraStic mappings observed on Bubble; do not change unrelated mmap calls.
 */

...
if [ "${1:-}" != "--inside" ]; then
    image=${1:-$repo_root/output/image/bubble-frontend-probe/plumOS-Bubble-0.1.0-dev-frontend-retroarch-probe.img}
...
if [ -f "$personalized_manifest" ] && \
    grep -q '^format=plumos-bubble-personalized-frontend-probe-v1$' \
...
grep -q '^format=plumos-bubble-frontend-probe-image-v1$' "$base_manifest"
...
grep -q '^format=plumos-bubble-frontend-runtime-probe-v1$' "$verify/runtime.manifest"
grep -q '^authorized=yes$' "$verify/runtime.manifest"
grep -q '^app_layer=frontend,retroarch,quicknes$' "$verify/runtime.manifest"
...
grep -q '^format=plumos-bubble-frontend-probe-image-v1$' "$verify/plumos-image.manifest"
...
grep -q '^libretro_core=quicknes$' "$verify/plumos-image.manifest"
grep -q '^roms_bios_included=no$' "$verify/plumos-image.manifest"
...
jq -e '.device == "bubble" and .roms_bios_included == false' "$app/manifest.json" >/dev/null

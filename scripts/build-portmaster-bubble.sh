...
PACKAGE_DIR="${ROOT_DIR}/package/portmaster-bubble"
...
Builds the plumOS Bubble PortMaster package from a pinned official release.

Environment:
  PLUMOS_BUBBLE_PORTMASTER_OUT         Output directory; default output/portmaster/bubble.
...
while IFS=':' read -r soname pattern; do
    ...
    [ -n "$source_library" ] && [ -f "$source_library" ] || {
        printf 'error: PortMaster package-local runtime is missing: %s\n' \
            "$soname" >&2
        exit 1
    }
    install -m 0644 "$source_library" \
        "$stage_dir/plumos/apps/portmaster/adapter/lib/aarch64/$soname"
done <<'EOF'
libSDL2_image-2.0.so.0:libSDL2_image-2.0.so.*
libSDL2_ttf-2.0.so.0:libSDL2_ttf-2.0.so.*
libz.so.1:libz.so.1.*
liblzma.so.5:liblzma.so.5.*
liblz4.so.1:liblz4.so.1.*
libzstd.so.1:libzstd.so.1.*
libFLAC.so.12:libFLAC.so.12.*
libmpg123.so.0:libmpg123.so.0.*
libopusfile.so.0:libopusfile.so.0.*
...
{
  "name": "plumOS PortMaster for Bubble",
  ...
  "compatibility_profile": "miyoo-flip pending per-port Bubble validation",
...
component=portmaster
target=bubble
compatibility_profile=miyoo-flip
adapter_version=${ADAPTER_VERSION}

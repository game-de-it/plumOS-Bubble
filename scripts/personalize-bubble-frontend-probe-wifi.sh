...
if [ "${1:-}" != "--inside" ]; then
    ...
    exec docker run --rm --platform linux/arm64 \
        ...
        -w /work "$tools_image" \
        ./scripts/personalize-bubble-frontend-probe-wifi.sh --inside
fi

base=${BASE_IMAGE:?}
output=${OUTPUT_IMAGE:?}
incoming=$output.incoming
work=/work/work/bubble-frontend-probe-wifi-personalize
case "$work" in /work/work/bubble-frontend-probe-wifi-personalize) ;; *) exit 2;; esac
cleanup() {
    rm -f "$incoming"
}
...
echo "bubble_frontend_probe_wifi_personalize=result-ok output=$output sha256=$output_sha"

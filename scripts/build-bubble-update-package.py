#!/usr/bin/env python3
"""Build signed plumOS Bubble Runtime update archives."""
...
def build_runtime(args: argparse.Namespace) -> tuple[dict[str, Any], list[tuple[Path, dict[str, Any]]]]:
    ...
    manifest: dict[str, Any] = {
        ...
        "system_abi_min": args.system_abi,
        "system_abi_max": args.system_abi,
...
def main() -> None:
    ...
    parser.add_argument("--type", choices=("runtime",), required=True)
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--base-dir", type=Path)
    parser.add_argument("--version", required=True)
    parser.add_argument("--base-version", default="*")
    parser.add_argument("--system-abi", default="1")
    parser.add_argument("--runtime-abi", default="1")
    parser.add_argument("--vendor-runtime", default=VENDOR_RUNTIME)
    parser.add_argument(
        "--signing-key",
        type=Path,
        default=Path(os.environ.get(
            "PLUMOS_BUBBLE_UPDATE_SIGNING_KEY",
            "artifacts/update-signing/plumos-bubble-ed25519-private.pem",
        )),
    )
    ...
    output = args.output_dir / f"plumos-bubble-{args.type}-{args.version}.tar.gz"

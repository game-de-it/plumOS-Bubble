...
if [ "${1:-}" != "--inside" ]; then
    ...
    exec docker run --rm --platform linux/arm64 \
        ...
        ./scripts/build-bubble-external-initramfs-probe-image.sh --inside
fi
...
out_dir=$repo_root/output/image/bubble-external-probe
work=$repo_root/work/bubble-external-initramfs-probe-image
...
case "$work" in /work/work/bubble-external-initramfs-probe-image) ;; *) exit 2;; esac
...
format=plumos-bubble-external-initramfs-runtime-probe-v1
authorized=yes
device=bubble
version=$version
source_ref=$source_ref
...
format=plumos-bubble-external-initramfs-probe-image-v1
device=bubble
...
image_path=$out_dir/plumOS-Bubble-$version-external-initramfs-probe.img
...
echo "bubble_external_probe_image=result-ok image=$image_path sha256=$image_sha"

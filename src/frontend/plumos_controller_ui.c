...
static int runtime_device_is_bubble(void);
...
static void init_device_settings(struct device_settings *device) {
  ...
  if (runtime_device_is_bubble()) {
    default_model = "GKD Bubble";
    default_gpu = "Mali-G52 isolated / CPU DRM";
  } else {
    default_model = runtime_device_is_mf() ? "Miyoo Flip" : "POWKIDDY V90S";
    default_gpu = runtime_device_is_mf() ? "Mali-G52 / CPU DRM" : "PowerVR GE8300";
  }
...
static void setting_help_lines(const struct ui_state *ui,
  ...
  if (strcmp(id, "ui_mode") == 0) {
    ...
    if (strcmp(id, "network_wifi_enabled") == 0) {
      ...
      copy_string(line2, line2_size, "Home is /storage; ON/OFF persists after reboot.");
      ...
      copy_string(line2, line2_size, "FTP home is /storage.");
